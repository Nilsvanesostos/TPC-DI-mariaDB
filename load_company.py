...
SET DimCompany.EndDate = 
    (SELECT EndDate FROM ( 
        ...
        JOIN DimCompany ON s1.SK_CompanyID = DimCompany.SK_CompanyID) as SubqueryOne);\n
"""
update_sdc_dimcompany_query_2 = """
UPDATE DimCompany
SET DimCompany.IsCurrent = 
    (SELECT false FROM ( 
        SELECT *
        FROM sdc_dimcompany s1
        JOIN sdc_dimcompany s2 ON (s1.RN = (s2.RN - 1) AND s1.CompanyID = s2.CompanyID)
        JOIN DimCompany ON s1.SK_CompanyID = DimCompany.SK_CompanyID) as SubqueryTwo)
WHERE EXISTS ( 
        SELECT *
        FROM sdc_dimcompany s1
        JOIN sdc_dimcompany s2 ON (s1.RN = (s2.RN - 1) AND s1.CompanyID = s2.CompanyID)
        JOIN DimCompany ON s1.SK_CompanyID = DimCompany.SK_CompanyID);\n
"""
