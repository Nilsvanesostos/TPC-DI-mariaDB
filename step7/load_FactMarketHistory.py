...
txt=open('FactMarketHistory.sql', 'w')

print('Loading DimMarketHistory...')
insert_dimmarkethistory_query = f"""
CREATE INDEX if not exists idx_sdailymarket_symb_date_high_low_close_vol ON S_Daily_Market (DM_S_SYMB, DM_DATE, DM_HIGH, DM_LOW, DM_CLOSE, DM_VOL);
CREATE INDEX if not exists idx_dimdate_datevalue ON DimDate (DateValue);
CREATE INDEX if not exists idx_dimsecurity_symbol_effectivedate_enddate ON DimSecurity (Symbol, EffectiveDate, EndDate);
CREATE INDEX if not exists idx_financial_sk_companyid_basic_eps ON Financial (SK_CompanyID, FI_BASIC_EPS);
...
txt.write(insert_dimmarkethistory_query)
txt.write('\n')
print('Done.')
